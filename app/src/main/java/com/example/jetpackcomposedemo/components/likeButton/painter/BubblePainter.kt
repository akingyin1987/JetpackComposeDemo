...
import com.example.jetpackcomposedemo.components.likeButton.BubbleColor
import com.example.jetpackcomposedemo.components.likeButton.degToRad
import com.example.jetpackcomposedemo.components.likeButton.forEach
 ...
 * @Description:
 * @author: aking
 * @CreateDate: 2023/1/8 13:06
 * @UpdateUser: 更新者
 * @UpdateDate: 2023/1/8 13:06
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 */
...
private fun DrawScope.drawOuter(
    ...
    val start = outerBubblesPositionAngle / 4.0f * 3.0f
    bubblesCount.forEach {
        val cx = centerX + currentRadius1 * cos(degToRad(start + outerBubblesPositionAngle * it)).toFloat()
        val cy = centerY + currentRadius1 * sin(degToRad(start + outerBubblesPositionAngle * it)).toFloat()
        drawCircle(
            color = bubbleColor.next(),
            center = Offset(cx, cy,),
